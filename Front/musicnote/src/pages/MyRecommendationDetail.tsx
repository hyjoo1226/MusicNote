...
export default function MyRecommendationDetail() {
  ...
  const { domain } = useParams();
	...
	const likeHandler = (id: number) => {
		setMovies(movies.map(movie => movie.id === id ? { ...movie, is_liked: !movie.is_liked } : movie));
	}
	...
	return (
		<div className="text-white w-full min-h-screen">
			...
			{isModalOpen && selectedMovie && (
				<div 
					className="w-screen h-screen bg-black/50 fixed top-0 left-0 z-50 flex items-end justify-center"
