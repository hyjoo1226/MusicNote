...
export default function MovieCarousel({ movies, getGenreNames }: MovieCarouselProps) {
  ...
  return (
    <>
      <Slider {...settings}>
        {movies.map((movie) => (
          <div key={movie.id} className="relative h-[400px] xs:h-[68vh] rounded-lg overflow-hidden">
            <img
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-black via-black/70 to-transparent">
              <div className="absolute bottom-0 left-0 right-0 p-8">
                ...
                <div className="flex items-center gap-4 text-white/90 mb-2">
                  <span className="text-xl text-light-gray">{movie.release_date.split('-')[0]}</span>
                ...
                <div className="flex flex-wrap gap-2">
                  {getGenreNames(movie.genre_ids).map((genre, index) => (
                    <span key={index} className="px-3 py-1 bg-level3 text-white rounded-full text-sm">
                      {genre}
                    </span>
                  ))}
                </div>
